...
var logClient = new Logger2("@firebase/database");
...
var enableLogging$1 = function(logger_, persistent) {
  ...
  if (logger_ === true) {
    logClient.logLevel = LogLevel2.VERBOSE;
...
var StatsCollection = class {
  ...
  incrementCounter(name3, amount = 1) {
    if (!contains2(this.counters_, name3)) {
      ...
      if (write.snap) {
        ...
        if (pathContains(treeRoot, writePath)) {
          ...
          if (pathIsEmpty(relativePath)) {
            ...
            const child4 = safeGet2(write.children, pathGetFront(relativePath));
            if (child4) {
...
var version2 = "1.0.20";
var SDK_VERSION4 = "";
function setSDKVersion2(version3) {
  SDK_VERSION4 = version3;
}
...
var log2 = function(...varArgs) {
  if (firstLog_2 === true) {
    firstLog_2 = false;
    if (logger2 === null && SessionStorage2.get("logging_enabled") === true) {
...
var PriorityIndex2 = class extends Index2 {
  ...
  makePost(indexValue, name3) {
    ...
    return new NamedNode2(name3, new LeafNode2("[PRIORITY-POST]", priorityNode));
  }
  /**
   * @returns String representation for inclusion in a query spec
   */
...
var PathIndex2 = class extends Index2 {
  ...
  makePost(indexValue, name3) {
    ...
    return new NamedNode2(name3, node);
  }
  maxPost() {
...
var ValueIndex2 = class extends Index2 {
  ...
  makePost(indexValue, name3) {
    const valueNode = nodeFromJSON2(indexValue);
    return new NamedNode2(name3, valueNode);
  }
  /**
   * @returns String representation for inclusion in a query spec
   */
...
var RangedFilter2 = class _RangedFilter {
  ...
  updateFullNode(oldSnap, newSnap, optChangeAccumulator) {
    ...
    const self = this;
    newSnap.forEachChild(PRIORITY_INDEX2, (key, childNode) => {
      if (!self.matches(new NamedNode2(key, childNode))) {
...
function writeTreeDefaultFilter_2(write) {
  return write.visible;
}
    ...
    if (filter(write)) {
      ...
      if (write.snap) {
        if (pathContains2(treeRoot, writePath)) {
          ...
          compoundWrite = compoundWriteAddWrite2(compoundWrite, relativePath, write.snap);
        } else if (pathContains2(writePath, treeRoot)) {
          relativePath = newRelativePath2(writePath, treeRoot);
          compoundWrite = compoundWriteAddWrite2(compoundWrite, newEmptyPath2(), write.snap.getChild(relativePath));
        } else ;
      } else if (write.children) {
        if (pathContains2(treeRoot, writePath)) {
          relativePath = newRelativePath2(treeRoot, writePath);
          compoundWrite = compoundWriteAddWrites2(compoundWrite, relativePath, write.children);
        } else if (pathContains2(writePath, treeRoot)) {
          relativePath = newRelativePath2(writePath, treeRoot);
          if (pathIsEmpty2(relativePath)) {
            compoundWrite = compoundWriteAddWrites2(compoundWrite, newEmptyPath2(), write.children);
          } else {
            const child4 = safeGet(write.children, pathGetFront2(relativePath));
            if (child4) {
  ...
  if (!writeIdsToExclude && !includeHiddenWrites) {
    ...
    if (!includeHiddenWrites && compoundWriteIsEmpty2(merge2)) {
      ...
      if (!includeHiddenWrites && completeServerCache == null && !compoundWriteHasCompleteWrite2(merge2, newEmptyPath2())) {
        ...
        const filter = function(write) {
          return (write.visible || includeHiddenWrites) && (!writeIdsToExclude || !~writeIdsToExclude.indexOf(write.writeId)) && (pathContains2(write.path, treePath) || pathContains2(treePath, write.path));
        };
...
function viewProcessorAckUserWrite2(viewProcessor, viewCache, ackPath, affectedTree, writesCache, completeCache, accumulator) {
  ...
  if (affectedTree.value != null) {
    if (pathIsEmpty2(ackPath) && serverCache.isFullyInitialized() || serverCache.isCompleteForPath(ackPath)) {
      ...
      serverCache.getNode().forEachChild(KEY_INDEX2, (name3, node) => {
        changedChildren = changedChildren.set(new Path2(name3), node);
      });
...
function syncTreeAckUserWrite2(syncTree, writeId, revert = false) {
  const write = writeTreeGetWrite2(syncTree.pendingWriteTree_, writeId);
  ...
  if (!needToReevaluate) {
    ...
    return syncTreeApplyOperationToSyncPoints_2(syncTree, new AckUserWrite2(write.path, affectedTree, revert));
  }
}
...
var validateUrl2 = function(fnName, parsedUrl) {
  const pathString = parsedUrl.path.toString();
  if (!(typeof parsedUrl.repoInfo.host === "string") || parsedUrl.repoInfo.host.length === 0 || !isValidKey2(parsedUrl.repoInfo.namespace) && parsedUrl.repoInfo.host.split(":")[0] !== "localhost" || pathString.length !== 0 && !isValidRootPathString2(pathString)) {
...
function repoOnDataUpdate2(repo, pathString, data, isMerge, tag) {
  ...
  if (tag) {
    if (isMerge) {
      const taggedChildren = map2(data, (raw) => nodeFromJSON2(raw));
    ...
    const changedChildren = map2(data, (raw) => nodeFromJSON2(raw));
...
function push(parent, value) {
  ...
  const thenablePushRef = child2(parent, name3);
  const pushRef = child2(parent, name3);
  let promise;
  if (value != null) {
...
function enableLogging(logger3, persistent) {
  enableLogging$12(logger3, persistent);
}
...
function _initStandalone({ app, url, version: version3, customAuthImpl, customAppCheckImpl, nodeAdmin = false }) {
  setSDKVersion2(version3);
